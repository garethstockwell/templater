...
public:
	...
	CWsScreenDevice& ScreenDevice() const { return *iScr; }
...
typedef CRendererEgl* (*RendererFactoryFunctionL)(RWindow&);
...
void CWsApp::ConstructL()
	...
	RendererFactoryFunctionL factoryFunctionL = CRendererVGLine::NewL;
	if (rendererName)
		{
		if (!rendererName->Des().Compare(CRendererGLES1Cube::Name()))
			{
			factoryFunctionL = CRendererGLES1Cube::NewL;
			}
		if (!rendererName->Des().Compare(CRendererVGLine::Name()))
			{
			factoryFunctionL = CRendererVGLine::NewL;
			}
		CleanupStack::PopAndDestroy(rendererName);
		}

	iRenderer = (*factoryFunctionL)(iAppView->Window());
	}
